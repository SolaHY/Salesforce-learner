// Data Cloud (Data 360) Consultant の模擬試験。
// 公式の練習試験を同梱していないため、単元別の全問題（src/certs/datacloud/questions/*）を
// 出題プールとし、ドメインを均等分散させたうえで本番想定の 60 問ずつに分割する。
// 決定論的に構成するので、リロードしても同じ回は同じ問題セットになる。
import { questions } from './quizzes'
import { domains } from './domains'

export const MOCK_SIZE = 60 // 1回あたりの問題数（本番は60問＋採点対象外5問）
export const PASS_PCT = 70 // 合格ライン（公式試験ガイド）
export const PASS_COUNT = 42 // 60問中の目安（70%）
...
function buildMocks() {
  ...
  for (let m = 0; m < n; m++) {
    ...
    cursor += size
    mocks.push(buildMock({ id: m + 1, title: `模試 ${m + 1}`, items, tag: '全6単元ミックス' }))
  }
  return mocks
}
